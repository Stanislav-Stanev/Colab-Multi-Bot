...
**Architecture:** Single source of truth is `fraud_multi_agent.py` in jupytext percent format (VS Code runs it as interactive cells; pytest imports it as a module with demos disabled via `FRAUD_SKIP_DEMOS=1`). `Fraud_Detection_Multi_Agent.ipynb` is generated from it with jupytext. Graph: `intake → fraud_analyst → compliance_officer → human_review(interrupt) → execute/cancel/revise-loop`.

**Tech Stack:** Python 3.11+, langgraph ≥0.4, langchain, langchain-anthropic (model `claude-sonnet-5`), python-dotenv, jupytext (build only), pytest (dev only).

## Global Constraints

- No API keys in code, ever. Secrets via `get_secret()`: Colab `userdata` → `.env` → `os.environ`.
- Notebook must run top-to-bottom in Colab with only the first `%pip install` cell as setup.
- All tools self-contained (mock data); the only external service is the Anthropic API.
- Demo cells guarded by `if not SKIP_DEMOS:` so pytest can import the file without LLM calls.
- Deterministic synthetic data (fixed timestamps, no `random` without seed).
- Spec: `docs/superpowers/specs/2026-09-10-fraud-detection-multi-agent-design.md`.

---

### Task 1: Scaffolding + notebook skeleton (header, install, secrets)

**Files:**
- Create: `.gitignore`, `.env.example`, `requirements-dev.txt`, `fraud_multi_agent.py`, `tests/conftest.py`, `tests/test_setup.py`

**Interfaces:**
- Produces: importable module `fraud_multi_agent` with `get_secret(name: str) -> str`, `SKIP_DEMOS: bool`, `MODEL_NAME: str`, `get_llm() -> ChatAnthropic` (lazy).

- [ ] **Step 1: Create `.gitignore`, `.env.example`, `requirements-dev.txt`**
...
`.env.example`:
```
# Copy to .env and fill in (VS Code / local). In Google Colab use Secrets (key icon) instead.
...
- [ ] **Step 2: Write failing test** — `tests/conftest.py`:

```python
import os, sys
os.environ["FRAUD_SKIP_DEMOS"] = "1"          # must be set BEFORE the module is imported
...
- [ ] **Step 3: Run to verify failure** — `python -m pytest tests/ -v` → FAIL (`ModuleNotFoundError: fraud_multi_agent`)

- [ ] **Step 4: Create `fraud_multi_agent.py`** (percent format; markdown header, install cell, secrets/config):
...
- [ ] **Step 5: Run tests** — `python -m pytest tests/ -v` → 3 PASS
- [ ] **Step 6: Commit** — `git add -A && git commit -m "feat: scaffolding, env-agnostic secrets, notebook skeleton"`

---

### Task 2: Synthetic dataset + the three tools

**Files:**
- Modify: `fraud_multi_agent.py` (append cells)
- Create: `tests/test_tools.py`

**Interfaces:**
- Produces: `CUSTOMER_DB: dict[str, dict]`; LangChain tools `fetch_customer_transactions(customer_id: str) -> str` (JSON), `calculate_risk_score(transactions_json: str) -> str` (JSON: `{"risk_score": int, "triggered_rules": [...], "summary": str}`), `check_sanctions_list(customer_name: str) -> str` (JSON); plus plain helpers `_fetch(customer_id) -> dict`, `_score(transactions: list[dict]) -> dict` used by tests.
...
def test_sanctions_hit_and_miss():
    assert json.loads(m.check_sanctions_list.invoke({"customer_name": "Viktor Baranov"}))["match"] is True
    assert json.loads(m.check_sanctions_list.invoke({"customer_name": "Maria Petrova"}))["match"] is False
```

- [ ] **Step 2: Run** → FAIL (`AttributeError: _fetch`)

- [ ] **Step 3: Append dataset + tools cells to `fraud_multi_agent.py`:**
...
def _score(transactions: list) -> dict:
    ...
    rules, score = [], 0
    times = sorted(datetime.fromisoformat(t["timestamp"]) for t in transactions)
    amounts = [t["amount_eur"] for t in transactions]

    # card testing: >=4 payments under 2 EUR within one hour
    small = [dt for dt, t in zip(times, sorted(transactions, key=lambda x: x["timestamp"]))
             if t["amount_eur"] < 2.0]
    ...
    for i in range(len(times)):
        if sum(1 for t2 in times if 0 <= (t2 - times[i]).total_seconds() <= 600) >= 5:
            rules.append("velocity"); score += 30; break
    # geo anomaly: >=3 countries within 24h
    if len({t["country"] for t in transactions}) >= 3 and \
       (times[-1] - times[0]).total_seconds() <= 7 * 86400:
    ...
    # risky MCCs
    hit_mccs = sorted({RISKY_MCC[t["mcc"]] for t in transactions if t["mcc"] in RISKY_MCC})
    if hit_mccs:
        rules.append("risky_mcc:" + ",".join(hit_mccs)); score += 15
    # night-time card-not-present activity (02:00-05:59)
    if sum(1 for dt, t in zip(times, transactions) if 2 <= dt.hour < 6) >= 2:
        rules.append("night_activity"); score += 10

    score = min(score, 100)
    return {"risk_score": score, "triggered_rules": rules,
            "summary": f"{len(transactions)} transactions, total {sum(amounts):.2f} EUR, "
...
- [ ] **Step 4: Run tests** → all PASS (adjust rule thresholds if a profile misses its intended rule — data and rules must agree; fix data, not the assertion's intent)
- [ ] **Step 5: Commit** — `git commit -am "feat: synthetic dataset, risk engine and 3 LangChain tools"`

---

### Task 3: State model + agent nodes + graph with HITL
...
**Interfaces:**
- Consumes: `TOOLS`, `get_llm()`.
- Produces: `FraudWorkflowState` (TypedDict), `RiskAssessment`/`ComplianceReport` (Pydantic), node functions `intake`, `fraud_analyst`, `compliance_officer`, `human_review`, `execute_action`, `cancel_action`, `followup_qa`; routers `route_after_intake(state) -> str`, `route_after_review(state) -> str`; compiled `graph` with `MemorySaver` checkpointer.

- [ ] **Step 1: Write failing tests** — `tests/test_graph.py` (pure logic only — no LLM):
...
def test_route_after_review():
    assert m.route_after_review({"human_decision": {"type": "approve"}, "revision_count": 0}) == "execute_action"
    assert m.route_after_review({"human_decision": {"type": "feedback", "feedback": "x"}, "revision_count": 1}) == "compliance_officer"
    assert m.route_after_review({"human_decision": {"type": "reject"}, "revision_count": 0}) == "cancel_action"
    # revision guard: after 3 revisions feedback is treated as final approve-with-note
    assert m.route_after_review({"human_decision": {"type": "feedback", "feedback": "x"}, "revision_count": 3}) == "execute_action"
...
- [ ] **Step 2: Run** → FAIL (`AttributeError: _extract_customer_id`)

- [ ] **Step 3: Append state/agents/graph cells:**
...
def fraud_analyst(state: FraudWorkflowState) -> dict:
    ...
    assessment = get_llm().with_structured_output(RiskAssessment).invoke(
        msgs + [HumanMessage(content="Now produce your final structured risk assessment.")])
...
for name, fn in [("intake", intake), ("fraud_analyst", fraud_analyst),
...
for terminal in ("execute_action", "cancel_action", "followup_qa"):
...
- [ ] **Step 5: Commit** — `git commit -am "feat: state model, two agents, HITL graph with MemorySaver"`

---

### Task 4: `execute_workflow` / `resume_workflow` / `run_scenario`
...
**Interfaces:**
- Consumes: `graph`.
- Produces: `execute_workflow(user_request: str, thread_id: str | None = None) -> dict` with keys `status` (`"awaiting_human_review" | "completed"`), `thread_id`, `interrupt_payload`/`final_output`; `resume_workflow(thread_id: str, decision: dict) -> dict` (same shape); `run_scenario(title, user_request, decisions: list[dict], thread_id=None) -> dict`.

- [ ] **Step 1: Write failing test** — `tests/test_workflow_api.py` (LLM-free: monkeypatch graph):
...
def test_execute_workflow_detects_interrupt(monkeypatch):
    class FakeGraph:
        def invoke(self, inp, config=None):
            return {"__interrupt__": [type("I", (), {"value": {"report": "draft"}})()]}
...
- [ ] **Step 2: Run** → FAIL (`AttributeError: execute_workflow`)

- [ ] **Step 3: Append the core-function cells:**
...
- [ ] **Step 5: Commit** — `git commit -am "feat: execute_workflow / resume_workflow / run_scenario"`

---

### Task 5: The six demo test cases (notebook cells)

**Files:**
- Modify: `fraud_multi_agent.py` (append cells)

**Interfaces:**
- Consumes: `run_scenario`, `execute_workflow`.

- [ ] **Step 1: Append demo cells** (each guarded with `if not SKIP_DEMOS:`):
    ...
    t3 = run_scenario("Test 3: velocity pattern (feedback → revision → approve)",
                      ...
                      decisions=[{"type": "feedback",
                                  "feedback": "Blocking is too aggressive for a first incident. "
                                              "Recommend MONITOR and describe what would escalate it to BLOCK."},
                                 {"type": "approve"}])
...
- [ ] **Step 2: Verify module still imports cleanly** — `python -m pytest tests/ -v` → all PASS
- [ ] **Step 3: Commit** — `git commit -am "feat: six HITL demo test cases"`

---

### Task 6: Live end-to-end run (real LLM), fix issues

**Files:**
- Modify: `fraud_multi_agent.py` (only if bugs found)

- [ ] **Step 1: Ensure `ANTHROPIC_API_KEY` is available in `.env`** (ask the user if absent — blocking input).
- [ ] **Step 2: Run the whole file live:** `python fraud_multi_agent.py` won't run demos as a script guard isn't used — instead run: `python -c "import fraud_multi_agent"` **without** `FRAUD_SKIP_DEMOS` from a shell where the key is loaded, or execute the generated notebook (Task 7 does it via nbclient). Preferred: `python - <<'EOF'` snippet importing and running each scenario; capture output.
- [ ] **Step 3: Fix any live issues** (structured-output quirks, tool-arg shapes) using systematic-debugging; re-run pytest after each fix.
- [ ] **Step 4: Commit** — `git commit -am "fix: live-run adjustments"` (only if changes)

---

### Task 7: Generate the notebook, execute it, README, ZIP

**Files:**
- Create: `Fraud_Detection_Multi_Agent.ipynb` (generated), `README.md`
- Create: `build.py` (optional convenience) or use CLI

- [ ] **Step 1: Generate notebook:** `python -m jupytext --to ipynb fraud_multi_agent.py -o Fraud_Detection_Multi_Agent.ipynb`
- [ ] **Step 2: Execute it fully** (proves "working system"): `python -m jupytext --set-kernel python3 Fraud_Detection_Multi_Agent.ipynb` then `python -m nbclient` alternative: `jupyter execute` or `python -m jupytext --execute`. Use: `python -m jupytext --to ipynb --execute fraud_multi_agent.py -o Fraud_Detection_Multi_Agent.ipynb` with the key loaded in env. Verify outputs (agent traces, interrupts, final reports) are saved in the .ipynb.
- [ ] **Step 3: Write `README.md`:** scenario summary, architecture diagram, rubric mapping table, **setup for Colab** (upload .ipynb, add `ANTHROPIC_API_KEY` in Secrets, Run all) and **setup for VS Code** (clone, `pip install -r requirements-dev.txt`, `.env`, open the .ipynb *or* run `fraud_multi_agent.py` as interactive cells), test-case list.
- [ ] **Step 4: Build submission ZIP:** `powershell Compress-Archive` of `Fraud_Detection_Multi_Agent.ipynb` + `README.md` → `submission.zip` (zip is gitignored).
- [ ] **Step 5: Final pytest + commit** — `python -m pytest tests/ -v` → all PASS; `git add -A && git commit -m "feat: generated notebook, README, submission package"`
